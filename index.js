...
function Bank() {
  ...
  this.updateBalance = (amount) => {
    ...
    document.getElementById('balance').innerText = utils.formatNumber(
      this.balance
    );
  };
  ...
  this.takeALoan = () => {
    if (this.numberOfLoans > 0) {
      alert(
        'You are not eligable for a loan. Please pay of your existing loan.'
      );
      return false;
    }
...
function Work() {
  ...
  this.work = () => {
    this.salary += 100;
    document.getElementById('salary').innerText = utils.formatNumber(
      this.salary
    );
  };
...
function Laptops() {
  ...
  this.printLaptopInfo = (laptop) => {
    ...
    document.getElementById('price').innerText = utils.formatNumber(
      laptop.price
    );
